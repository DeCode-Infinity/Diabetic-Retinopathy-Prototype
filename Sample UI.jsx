...
function checkIsRetina(imgEl) {
  ...
  const avgAlpha = aSum/n;
  // Real fundus photos are opaque JPEGs — no transparency. An image with
  // meaningful transparency (icons, logos, PNG cutouts) isn't a photo at all.
  if (avgAlpha < 250) return false;
  ...
  const redDominant = rAvg > gAvg*1.15 && rAvg > bAvg*1.3; // fundus = red/orange
  const hasVignette = cornerDark >= 2;                      // corners near-black

  return redDominant && hasVignette;
}
...
export default function DRScreenAI() {
  ...
  const handleFile=useCallback((file)=>{
    ...
    rdr.onload=e=>{
      ...
      setStage("validating");                 // gatekeeper stage starts
      const img=new Image();
      let settled=false;
      const timeout=setTimeout(()=>{           // safety net: never hang forever
        if(!settled){ settled=true; setImgSrc(dataUrl); setStage("unsupported"); }
      },4000);
      img.onerror=()=>{
        if(settled)return; settled=true; clearTimeout(timeout);
        setImgSrc(dataUrl); setStage("unsupported");  // e.g. browser can't decode .tif
      };
      img.onload=()=>{
        if(settled)return; settled=true; clearTimeout(timeout);
